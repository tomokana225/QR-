...
const SubmissionChecker: React.FC<{
    ...
    return (
        <div className="flex flex-col lg:flex-row gap-6 lg:gap-10 h-full">
            <div className="w-full lg:w-80 flex-shrink-0 flex flex-col gap-6 lg:gap-8">
                <div className="bg-white p-6 lg:p-8 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100">
                    <div className="flex justify-between items-center mb-4">
                        ...
                        {activeSubmissionListId && (
                            <button onClick={() => onDeleteSubmissionList(activeSubmissionListId)} className="p-1.5 text-red-400 hover:bg-red-50 hover:text-red-600 rounded-lg transition-colors">
                                <TrashIcon className="w-4 h-4" />
                            </button>
                        )}
                    </div>
                    <div className="flex gap-2 mb-4">
                        <select value={activeSubmissionListId || ''} onChange={e => onSetActiveSubmissionListId(e.target.value)} className="flex-grow p-4 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-black outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all cursor-pointer w-full min-w-0">
                    ...
                    {activeSubmissionListId && (
                        <button onClick={handleExportCSV} className="w-full mb-6 py-3 px-4 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-700 flex items-center justify-center gap-2 hover:bg-white hover:border-indigo-300 hover:text-indigo-600 transition-all shadow-sm">
                            <DocumentArrowDownIcon className="w-4 h-4" />
                            Excel (CSV) 出力
                        </button>
                    )}
                    
                    <button onClick={onResetCurrentList} className="w-full py-3 px-4 border-2 border-slate-100 rounded-xl text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] hover:bg-red-50 hover:text-red-500 hover:border-red-100 transition-all flex items-center justify-center gap-2">
                ...
                <div className="bg-slate-900 p-6 lg:p-8 rounded-3xl shadow-2xl shadow-indigo-900/10">
                    ...
                    <form onSubmit={handleBarcodeSubmit} className="mb-6">
                        <input type="text" value={barcodeInput} onChange={e => setBarcodeInput(e.target.value)} placeholder="コードを手入力..." className="w-full p-4 bg-slate-800 border border-slate-700 rounded-2xl text-white text-sm outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/20 transition-all placeholder:text-slate-600" />
                    </form>
                    <button onClick={() => setIsCameraScannerOpen(true)} className="w-full py-5 bg-indigo-600 text-white rounded-2xl font-black text-sm flex items-center justify-center gap-3 hover:bg-indigo-500 transition-all shadow-xl shadow-indigo-500/30 active:scale-95">
                        カメラを起動
                    </button>
                </div>
                                ...
                                return (
                                    <tr key={s.id} className={`group transition-all ${timestamp ? 'bg-emerald-50/20' : 'bg-transparent hover:bg-slate-50'}`}>
                                        ...
                                        <td className="px-3 lg:px-6 py-3 lg:py-5">
                                            <button onClick={() => handleToggleSubmission(s.id, timestamp)} className={`inline-flex items-center gap-2 px-3 lg:px-4 py-2 rounded-xl text-[10px] lg:text-[11px] font-black uppercase tracking-wider transition-all active:scale-95 group/status shadow-sm whitespace-nowrap ${
                                                timestamp ? 'bg-emerald-50 text-emerald-700 border border-emerald-100 hover:bg-red-50 hover:text-red-700 hover:border-red-100' : 'bg-slate-50 text-slate-500 border border-slate-200 hover:bg-indigo-50 hover:text-indigo-700 hover:border-indigo-100'
                                            }`}>
                                                {timestamp ? (
                                                    <>
            ...
            <CameraScannerModal isOpen={isCameraScannerOpen} onClose={async () => setIsCameraScannerOpen(false)} onScanStudent={handleSubmissionScan} students={students} activeList={activeList} settings={settings} onSettingsChange={onSettingsChange} scannerId="qr-reader-submission" title="提出スキャナー" />
        </div>
    );
};
